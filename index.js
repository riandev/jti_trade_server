...
const app = express();
app.use(express.static("../jti_trade_client/build"));
app.use(cors());
...
// const MongoClient = require("mongodb").MongoClient;
const uri = "mongodb://127.0.0.1:27017/jti_trade";
// const uri =
//   "mongodb+srv://aktcl:01939773554op5t@cluster0.9akoo.mongodb.net/jti_trade?retryWrites=true&w=majority";
const client = new MongoClient(uri, {
  useNewUrlParser: true,
  useUnifiedTopology: true,
});
client.connect((err) => {
  const userCollection = client.db("jti_trade").collection("users");
  const adminCollection = client.db("jti_trade").collection("admins");
  const leadsCollection = client.db("jti_trade").collection("leads");
      ...
      .toArray((err, results) => {
        ...
        for (user in users) {
          output.push({
            ...
            initLeads: users[user].slice(0, 11).map((d) => {
              let datas = {};
              (datas.id = d._id),
                (datas.diid = d.DIID),
  ...
  app.get("*", (req, res) => {
    res.sendFile(
      path.join(__dirname, "../jti_trade_client/build", "index.html")
    );
  });
});
